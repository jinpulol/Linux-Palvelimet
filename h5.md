...
Annoin ensiksi luvan käyttäjille luoda kotisivuja komennolla ``sudo a2enmod userdir``, jonka jälkeen käynnistin Apachen uudelleen.
...
Otin myös vanhan 000-default.conf virtuaalipalvelimen pois vastaavasti ``a2dissite``-komentoa käyttäen. Tässä kohdin selaimeni ilmoitti Forbidden-virhettä, kun yritin yhdistää sivulleni. Lisäsin ``sudo chmod ugo+x $HOME $HOME/public_html/``-komennolla oikeat oikeudet kansiolleni. Tämän jälkeen lolleropyllero.onlinen etusivulla näkyi omassa kotihakemistossa oleva html-sivu.
