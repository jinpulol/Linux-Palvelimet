...
- Uuden kehitysympäristön luominen Pythonin paketeilla käy komennolla ``$ virtualenv --system-site-packages -p python3 env/``.
...
- Python package manageria (pip) tulee käyttää vain kehitysympäristössä **ilman** sudo-voimia.
...
- Uuden mallin lisääminen tapahtuu muokkaamalla crm-hakemistossa olevaa ``models.py``-tiedostoa:
  ...
  -     from django.db import models
        ...
        name = models.CharField(max_length=300)  # Customer-luokasta löytyy nyt name-attribuutti

        def _str_(self):    ## nämä kaksi viimeistä riviä muuttavat attribuutin merkkijonoksi
...
Kiireistä johtuen yritän tehdä tehtävän myöhemmin.
